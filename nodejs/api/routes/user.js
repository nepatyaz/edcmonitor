...
router.put('/', (req, res) => {
  if (token.cekToken(req.headers['authorization'])) {
    ...
    var query2 = "UPDATE users SET address1 = '" + req.body.address1 + "', address2 = '" + req.body.address2 + "', address3 = '" + req.body.address3 + "', email = '" + req.body.email + "', enabled = '" + req.body.enabled.data + "',  password = '" + hash + "', avatar = '" + req.body.avatar + "' WHERE `users`.`id` = " + req.body.id;
    console.log(query2);
...
router.post('/register', (req, res) => {
  ...
  if (token.cekToken(req.headers['authorization'])) {
    ...
    conDb.query("INSERT INTO `edcmondb`.`users` (`address1`,`address2`,`address3`,`branch`,`enabled`,`is_login`,`password`,`username`,`email`,`avatar`)VALUES(?,?,?,?,?,?,?,?,?,?)", [address1, address2, address3, branch, enabled, 0, hash, username, email, avatar], (err, rows, fields) => {
      if (!err) {
...
router.post('/enable', (req, res) => {
  ...
  if (token.cekToken(req.headers['authorization'])) {
    ...
    var sql = "UPDATE `users` SET `enabled` = " + value + " WHERE `users`.`id` = " + id;
    console.log(sql);

    conDb.query(sql, (err, rows, fields) => {
      if (!err) {
        if (rows.affectedRows > 0) {
          res.send(pesan.enableSucces);
        } else {
          res.status(400);
          res.send(pesan.enableGagal);
        }
      } else {
        var pesanError = Object.assign(pesan.gagalfetch, err);
        res.status(400);
